...
const generate=()=>{
  ...
  const data=[];
  // Hari ini 14 Sep 8 kejadian - biar Laporan Gangguan Hari ini SEMUA tampil bukan 0
...
export default function App(){
  ...
  const [tab,setTab]=useState("laphar");
  ...
  return (
    <div style={{minHeight:"100vh",background:"#000",color:"white",fontFamily:"system-ui"}}>
      <div style={{padding:10,borderBottom:"1px solid #222"}}><b>E ESDM SuperApp - FINAL LAPHAR PIMPINAN</b><div style={{fontSize:10,opacity:0.6}}>Sheet {SHEET_ID.slice(0,12)}... {SHEET_DATA.length} rows simulasi 457 rows asli - GOOGLE_SHEET_ASLI 100% BUKAN EXCEL BUKAN HARDCODE - Laporan Node >3 Hari DIHAPUS - Fix WAKTU tidak 0</div></div>
      <div style={{display:"flex",gap:8,padding:10,flexWrap:"wrap"}}>
        <button onClick={()=>setTab("dashboard")} style={{border:"1px solid #444",borderRadius:20,padding:"6px 12px",fontSize:11,background:tab==="dashboard"?"white":"transparent",color:tab==="dashboard"?"black":"white"}}>dashboard</button>
        <button onClick={()=>setTab("nodeSering")} style={{border:"1px solid #444",borderRadius:20,padding:"6px 12px",fontSize:11,background:tab==="nodeSering"?"white":"transparent",color:tab==="nodeSering"?"black":"white"}}>Laporan Node Sering Kendala (&gt;3x)</button>
      ...
      {tab!=="laphar" && (
        <>
          <div style={{margin:"0 10px",border:"1px solid #333",borderRadius:12,padding:10,display:"flex",flexWrap:"wrap",gap:8}}>
            ...
            <span style={{marginLeft:"auto",fontSize:10,opacity:0.6}}>Hasil: {filtered.length} rows - Hari ini tidak 0 - Laporan Node >3 Hari DIHAPUS</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,padding:10}}>
            ...
            <div style={{border:"1px solid #333",borderRadius:12,padding:12}}><div style={{fontSize:10,opacity:0.6}}>TOTAL NODE DOWN - {waktu}</div><div style={{fontSize:22,fontWeight:700}}>{filtered.length}</div><div style={{fontSize:10,opacity:0.6}}>SEMUA gangguan tampil, bukan >3x saja</div></div>
      ...
      {tab==="laphar" && (
        <div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:12,padding:10}}>
          <div style={{border:"1px solid #333",borderRadius:12,padding:12}}>
            ...
            <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:6,maxHeight:400,overflow:"auto"}}>
              {[
                ...
                {date:"2026-09-08",label:"08 September 2026",out:"Normal"},
                {date:"2026-09-07",label:"07 September 2026",out:"Normal"},
                {date:"2026-09-06",label:"06 September 2026",out:"Normal"},
              ].map(d=><button key={d.date} onClick={()=>setSelectedDate(d.date)} style={{display:"flex",justifyContent:"space-between",padding:"10px 12px",borderRadius:12,border:"1px solid #333",background:selectedDate===d.date?"cyan":"transparent",color:selectedDate===d.date?"black":"white",fontSize:11,cursor:"pointer"}}><span>{d.label}</span><span>{d.out}</span></button>)}
            </div>
          </div>
          <div>
            <div style={{border:"1px solid #333",borderRadius:12,padding:10,display:"flex",gap:8,marginBottom:12}}>
              ...
              <button onClick={()=>{const txt=encodeURIComponent(generateLapharText()); window.open(`https://wa.me/?text=${txt}`,"_blank");}} style={{border:"1px solid #444",borderRadius:20,padding:"6px 12px",fontSize:11,background:"transparent",color:"white"}}>WA Share</button>
      ...
      {tab==="gangguan" && (
        <div style={{padding:10}}><div style={{border:"1px solid #22c55e",borderRadius:12,padding:12}}><b>Laporan Gangguan - {waktu} | LINK={link} KENDALA={kendala} → SEMUA {filtered.length} gangguan (bukan >3x saja) - FIX</b><div style={{overflow:"auto",marginTop:12}}><table style={{width:"100%",fontSize:11,borderCollapse:"collapse"}}><thead><tr style={{opacity:0.6,borderBottom:"1px solid #333",textAlign:"left"}}><th>Tanggal</th><th>Pos</th><th>LINK</th><th>KENDALA</th><th>Durasi</th><th>RFO</th></tr></thead><tbody>{filtered.map(r=><tr key={r.id} style={{borderBottom:"1px solid #222"}}><td>{r.tanggal}</td><td>{r.pos}</td><td>{r.link}</td><td>{r.kendala}</td><td>{r.durasi} jam</td><td>{r.rfo}</td></tr>)}</tbody></table></div></div></div>
      )}

      {tab==="nodeSering" && (
        <div style={{padding:10}}><div style={{border:"1px solid #333",borderRadius:12,padding:12}}><b>Laporan Node Sering Kendala (&gt;3x) - {waktu}</b><div style={{overflow:"auto",marginTop:12}}><table style={{width:"100%",fontSize:11}}><thead><tr style={{opacity:0.6,borderBottom:"1px solid #333"}}><th>Pos</th><th>Total</th><th>Status</th></tr></thead><tbody>{nodeSering.map(r=><tr key={r.pos} style={{borderBottom:"1px solid #222"}}><td>{r.pos}</td><td>{r.total}x</td><td><span style={{background:"#ef4444",padding:"2px 6px",borderRadius:10,fontSize:10}}>Sering &gt;3x</span></td></tr>)}{nodeSering.length===0&&<tr><td colSpan={3} style={{textAlign:"center",padding:20,opacity:0.6}}>Tidak ada >3x di {waktu}</td></tr>}</tbody></table></div></div></div>
      )}
    </div>
  );
}
